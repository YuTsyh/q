"""Tests for the Volatility Mean-Reversion strategy.

Validates:
- Garman-Klass volatility helpers
- Composite scoring
- Regime gating
...
class TestVolMeanReversionHelpers:
    """Unit tests for standalone helper functions."""

    def test_garman_klass_var_length(self) -> None:
        bars = _make_bars("X", 30)
        gk = _garman_klass_var(bars)
        assert len(gk) == len(bars)

    def test_garman_klass_var_non_negative(self) -> None:
        bars = _make_bars("X", 30)
        gk = _garman_klass_var(bars)
        for v in gk:
            assert v >= 0.0

    def test_garman_klass_var_deterministic(self) -> None:
        bars = _make_bars("X", 30, seed=7)
        assert _garman_klass_var(bars) == _garman_klass_var(bars)

    def test_garman_klass_var_zero_price(self) -> None:
        """Bars with zero prices should produce zero variance."""
        bar = OhlcvBar(
            inst_id="Z",
            ts=_T0,
            open=Decimal("0"),
...
# ---------------------------------------------------------------------------
# Tests: VolMeanReversionAlpha
# ---------------------------------------------------------------------------


class TestVolMeanReversionAlpha:
    """Core allocation logic."""

    def test_allocate_returns_dict(self) -> None:
        strategy = VolMeanReversionAlpha()
    ...
    def test_max_weight_respected(self) -> None:
        config = VolMeanReversionConfig(max_position_weight=0.20)
    ...
    def test_gross_exposure_respected(self) -> None:
        config = VolMeanReversionConfig(gross_exposure=0.8)
        strategy = VolMeanReversionAlpha(config)
        insts = {f"I{i}": _make_bars(f"I{i}", 80, seed=i) for i in range(5)}
        fundings = {f"I{i}": _make_funding(f"I{i}", 80, seed=i) for i in range(5)}
        weights = strategy.allocate(insts, fundings)
    ...
    def test_successive_calls_update_state(self) -> None:
        strategy = VolMeanReversionAlpha()
        bars = _make_bars("A", 80, seed=1)
        funding = _make_funding("A", 80, seed=1)
    ...
    def test_zero_price_bars_handled(self) -> None:
        """Bars with zero close should not crash the strategy."""
    ...
    def test_crisis_regime_returns_flat(self) -> None:
        config = VolMeanReversionConfig(
            crisis_exposure=0.0,
    ...
    def test_drawdown_circuit_breaker(self) -> None:
        strategy = VolMeanReversionAlpha(
            VolMeanReversionConfig(dd_threshold=0.02),
        )
...
class TestCreateVolMRAllocator:
    """The factory function must produce a valid allocator callable."""

    def test_factory_returns_callable(self) -> None:
        alloc = create_vol_mean_reversion_allocator()
...
class TestVolMRBacktestIntegration:
